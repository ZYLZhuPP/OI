...
int getG_(int u, int pre, int sum){
    msiz[u]=0; siz[u]=1;
    int ret=0;
    for(auto e: es[u]) if(e.to^pre && !vis[e.to]){
        int v=e.to;
        ret|=getG_(v, u, sum);
        siz[u]+=siz[v];
        chkmax(msiz[u], siz[v]);
    }
    if(ret){Siz[u]=sum-msiz[u]; return ret;}
    if(max(msiz[u], sum-siz[u])<=sum/2) return u;
    Siz[u]=siz[u]; return 0;
}
...
void solve(int u){
    ...
    for(auto e: es[u]) if(!vis[e.to]){
        int v=e.to; dis[v]=dis[u]+e.w;
        init_dis(v, u);
        for(int i=1; i<=m; i++) if(!ans[i]){
            for(auto w: now) if(ques[i]>=w) ans[i]|=judge[ques[i]-w];
        }
        for(auto w: now) judge[w]=1, change.emplace_back(w); vector<int >().swap(now);
    }
    for(auto w: change) judge[w]=0; vector<int >().swap(change);
}

void Divide(int rt){
    int G=getG(rt);
    solve(G);
    for(auto e: es[G]) if(!vis[e.to]) Divide(e.to);
}
