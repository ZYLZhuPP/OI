...
int main()
{
    cam.SetCoordinateSystem(Matrix34CameraLookAt(MakePos(0.0, 2.0, 0.0), MakePos(1.0, 2.0, 0.0)));
    ...
    model_pack.PushModel(model_sph);
    for (int i = 2; i < 8; i++)
    ...
    scene->Render("Panorama", 16, 0, false, 1);
    delete scene;
}