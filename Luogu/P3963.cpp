...
int main() {
    ...
    sort(a + 1, a + n + 1);
    For (i, 1, n) L.upd(1, 0, 1e5, a[i].v, 1);
    int ans = 0;
    rFor (i, n, 1) {
        L.upd(1, 0, 1e5, a[i].v, -1);
        while (min(n - i, i - 1) >= ans && L.query(1, 0, 1e5, ans) + a[i].v + R.query(1, 0, 1e5, ans) <= m) f[ans << 1 | 1] = a[i].w, ans++;
        R.upd(1, 0, 1e5, a[i].v, 1);
    }
    printf("%d", f[K]);
