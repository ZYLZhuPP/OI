...
int n, m, K, P, dw[N], _[N], ivf[N], inv[N], ans, mip[N], pri[N / 10], psz, id[N];

inline void init() {
    dw[0] = 1; For (i, 1, K) dw[i] = ml(dw[i - 1], n - i + 1);
    _[0] = 1; For (i, 1, K) _[i] = ml(_[i - 1], P);
...
int main() {
    io >> n >> m >> K; P = qPow(m, Mo - 2);
    init();
    int x = min(n, K), y = 1, z = 1;
    rFor (i, x, 0) {
        if (i < x) {
            mul(y, ml(n - i - 1, inv[K - i]));
            z = mo(ml(z, mo(1 - P)) + ml(y, ((K-i)&1? -1: 1) * _[K - i]));
        }
        add(ans, ml(ml(id[i], ml(dw[i], ivf[i])), ml(_[i], z)));
    }
    printf("%d", ans);
