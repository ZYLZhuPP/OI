...
int n, fac[N], ivf[N], a[N], ans = 1;

inline void init() {
    fac[0] = 1; For (i, 1, N - 1) fac[i] = ml(fac[i - 1], i);
    ivf[N - 1] = qPow(fac[N - 1], Mo - 2); rFor (i, N - 1, 1) ivf[i - 1] = ml(ivf[i], i);
}
...
int main() {
    init();
    io >> n;
    For (i, 1, n) io >> a[i], a[i] >>= 1;
    For (i, 1, n - 1) mul(ans, a[i]<a[i + 1]? C(a[i + 1] - 1, a[i] - 1): C(a[i], a[i + 1]));
    printf("%d", ans);
