...
ll n, h[N], st[N], ed[N], L, R, H, ll, rr;
vector<ll > ans;
stack<ll > q;
...
int main() {
	...
	while (l <= r) {
		ll mid = (l + r) >> 1;
		if (pre(mid) < R) rr = mid, l = mid + 1;
		else r = mid - 1;
	}
	
	if (ll == rr) {
	    for (int i = 0; i <= R - L; ++i) io << ll << ' ';
	    return 0;
	}
	
	ll cntl = pre(ll + 1) - L + 1;
	ll cntr = R - pre(rr);
		
	for (int i = 1; i <= n; ++i) {
		ll len = ed[i] - st[i] + 1;
		ll a = (ll + 1 + h[i] - 1) / h[i], b = (rr - 1) / h[i];
	...
	for (int i = 1; i <= cntl; ++i) io << ll << ' ';
	sort(ans.begin(), ans.end());
	for (auto &x : ans) io << x << ' ';
	for (int i = 1; i <= cntr; ++i) io << rr << ' ';
