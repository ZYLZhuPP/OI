...
int main() {
    ...
    For (i, 1, n) s[i] = a[n - i + 1];
    SA(s, n, m);
    For (i, 1, n) if (sa[i] > 2) { p1 = sa[i]; break; }

    int nn = p1 - 1;
    For (i, 1, nn) s[i + nn] = s[i];
    SA(s, nn * 2, m);
    For (i, 1, nn * 2) if (sa[i] <= nn) { p2 = sa[i]; break; }

    rFor (i, n - p1 + 1, 1) printf("%d\n", b[a[i]]);
    For (i, p2, p2 + nn - 1) printf("%d\n", b[s[i]]);
