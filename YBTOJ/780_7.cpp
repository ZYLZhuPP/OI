...
int n, m, S, a[N], sa[N], cnt[_N], sb[_N], be[N], bl[_N], br[_N];
int ans1[N], ans2[N];
...
inline void solve(int l, int r, int id) {
    int L = be[l], R = be[r];
    For (i, l, min(br[L], r)) if (sa[i]) ans1[id] += sa[i], ans2[id]++;
    For (i, L + 1, R - 1) ans1[id] += sb[i], ans2[id] += cnt[i];
    if (L ^ R) For (i, bl[R], r) if (sa[i]) ans1[id] += sa[i], ans2[id]++;
}
