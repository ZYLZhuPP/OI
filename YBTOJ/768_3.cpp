...
namespace Heavy {
    ...
    void init(int rt = 1) {
        ...
        For (i, 1, n) Seg::upd(Seg::rt[c[i]], 1, n, dfn[i], V(w[i]));
    }
    void upd(int u, const V &v) {
        Seg::upd(Seg::rt[c[u]], 1, n, dfn[u], v);
    }
    V query(int u, int v) {
        int &r = Seg::rt[c[u]]; V res;
        while (top[u] ^ top[v]) {
            if (dep[top[u]] < dep[top[v]]) swap(u, v);
            res = res + Seg::query(r, 1, n, dfn[top[u]], dfn[u]);
            u = fa[top[u]];
        }
        if (dep[u] < dep[v]) swap(u, v);
        res = res + Seg::query(r, 1, n, dfn[v], dfn[u]);
        return res;
    }
}
