...
#define goR rs, o[u].r, v
    ...
    inline int size(int rt) { return o[rt].sz; }
    inline void ins(int &rt, const V &v) {
        split_k(rt, rt, y, v);
        x = node(v);
        rt = merge(rt, merge(x, y));
    }
    inline void del_all_k(int &rt, const V &v) {
        split_k(rt, rt, x, v); split_k_(x, x, y, v);
        rt = merge(rt, y);
    }
    inline void del_a_k(int &rt, const V &v) {
        split_k(rt, rt, x, v); split_k_(x, x, y, v);
        x = merge(o[x].l, o[x].r);
        rt = merge(rt, merge(x, y));
    }
    inline void del_r(int &rt, const int &l, const int &r) {
        split_r(rt, rt, x, l - 1); split_r(x, x, y, r - l + 1);
        rt = merge(rt, y);
    }
    inline void del_r(int &rt, const int &k) { del_r(rt, k, k); }
    inline int count(int rt, const V &l, const V &r) {
        split_k(rt, rt, x, l); split_k_(x, x, y, r);
        return o[x].sz;
    }
    inline int count(int rt, const V &v) { return count(rt, v, v); }
    inline int rnk(int rt, const V &v) {
        split_k(rt, x, y, v);
        return o[x].sz + 1;
    }
    inline V pre(int rt, const V &v) {
        split_k(rt, x, y, v);
        int p = x; while (rs) p = rs;
        return p? o[p].v: -INT_MAX;
    }
    inline V suf(int rt, const V &v) {
        split_k_(rt, x, y, v);
        int p = y; while (ls) p = ls;
        return p? o[p].v: INT_MAX;
    }
    inline V kth(int rt, const int &k) {
