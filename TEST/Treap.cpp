...
#define goR rs, o[u].r, v
    ...
    inline int size() const { return o[rt].sz; }
    inline void ins(const V &v) {
        split_k(rt, rt, y, v);
        x = ++tot, o.pb(Node(v));
        rt = merge(rt, merge(x, y));
    }
    inline void del_all_k(const V &v) {
        split_k(rt, rt, x, v); split_k_(x, x, y, v);
        rt = merge(rt, y);
    }
    inline void del_a_k(const V &v) {
        split_k(rt, rt, x, v); split_k_(x, x, y, v);
        x = merge(o[x].l, o[x].r);
        rt = merge(rt, merge(x, y));
    }
    inline void del_r(const int &l, const int &r) {
        split_r(rt, rt, x, l - 1); split_r(x, x, y, r - l + 1);
        rt = merge(rt, y);
    }
    inline void del_r(const int &k) { del_r(k, k); }
    inline int count(const V &l, const V &r) {
        split_k(rt, rt, x, l); split_k_(x, x, y, r);
        int res = o[x].sz;
        rt = merge(rt, merge(x, y));
        return res;
    }
    inline int count(const V &v) { return count(v, v); }
    inline int rnk(const V &v) {
        split_k(rt, x, y, v);
        int res = o[x].sz + 1;
        rt = merge(x, y);
        return res;
    }
    inline V pre(const V &v) {
        split_k(rt, x, y, v);
        int p = x; while (rs) p = rs;
        rt = merge(x, y);
        return o[p].v;
    }
    inline V suf(const V &v) {
        split_k_(rt, x, y, v);
        int p = y; while (ls) p = ls;
        rt = merge(x, y);
        return o[p].v;
    }
    inline V kth(const int &k) {
